...
export function hasClearAttackPath(from: { x: number; y: number }, to: { x: number; y: number }, worldId: WorldId = 'neon_bastion') {
  const steps = Math.ceil(Math.hypot(to.x - from.x, to.y - from.y) / 35);
  for (let step = 1; step < steps; step++) {
    const fraction = step / steps;
    if (!isWorldPositionClear(from.x + (to.x - from.x) * fraction, from.y + (to.y - from.y) * fraction, 5, worldId)) return false;
  }
  return true;
}

export function moveTacticalEnemy(enemy: CoopEnemySnapshot, target: { x: number; y: number }, neighbors: readonly CoopEnemySnapshot[], elapsedMs: number, deltaMs: number, cachedClearAttackPath?: boolean, worldId: WorldId = 'neon_bastion') {
  ...
  if ((enemy.type === 'ranged' || enemy.type === 'elite') && (cachedClearAttackPath ?? hasClearAttackPath(enemy, target, worldId))) {
  ...
  for (const offset of [0, side * .55, -side * .55, side * 1.1, -side * 1.1, side * Math.PI / 2]) {
    ...
    if (!isWorldPositionClear(probeX, probeY, enemy.radius, worldId)) continue;
...
export function findEnemyDetour(enemy: CoopEnemySnapshot, target: { x: number; y: number }, worldId: WorldId = 'neon_bastion'): { x: number; y: number } | undefined {
  ...
  for (const offset of [side * Math.PI / 2, -side * Math.PI / 2, side * .85, -side * .85, Math.PI]) {
    ...
    if (!isWorldPositionClear(x, y, enemy.radius + 5, worldId) || !hasClearAttackPath(enemy, { x, y }, worldId)) continue;
