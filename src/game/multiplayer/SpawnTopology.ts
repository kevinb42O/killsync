...
export class SpawnTopology {
  ...
  constructor() {
    ...
    for (let y = MAP_MARGIN; y <= GAME_HEIGHT - MAP_MARGIN; y += GRID_STEP) {
      for (let x = MAP_MARGIN; x <= GAME_WIDTH - MAP_MARGIN; x += GRID_STEP) {
        if (isWorldPositionClear(x, y, 42)) nodes.push({ id: id++, x, y });
      }
    }
    this.nodes = nodes;
  }
  ...
  private isEligible(node: SpawnNode, cluster: EncounterCluster, players: readonly EncounterPlayer[], radius: number, occupied: readonly { x: number; y: number; radius: number }[]) {
    ...
    if (clusterDistance < MIN_CLUSTER_DISTANCE || clusterDistance > MAX_CLUSTER_DISTANCE || !isWorldPositionClear(node.x, node.y, radius + 10)) return false;
    if (!straightPathIsClear(node, cluster, radius)) return false;
...
function straightPathIsClear(node: SpawnNode, cluster: EncounterCluster, radius: number) {
  ...
  for (let step = 1; step < steps; step++) {
    ...
    if (!isWorldPositionClear(x, y, Math.max(16, radius))) return false;
  }
  return true;
}
