    ...
    <div className="absolute inset-0 z-[110] bg-[#05080e]">
      ...
      <div className="pointer-events-none absolute left-5 top-40 w-52 border border-white/15 bg-black/60 p-2 backdrop-blur-sm">
        ...
        {hud.squad.map(player => {
          const status = player.lifeState === 'alive' ? `${Math.ceil(player.health)}/${Math.ceil(player.maxHealth)}` : player.lifeState === 'downed' ? `DOWN ${Math.ceil(player.downedRemainingMs / 1000)}s` : 'OUT';
      ...
      {nearbyStation && <div className="absolute bottom-5 left-5 w-[min(390px,calc(100vw-2.5rem))] border border-cyan-300/35 bg-[#07111b]/95 p-4 shadow-[0_0_28px_rgba(34,211,238,.14)] backdrop-blur-md">
        <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200"><ShoppingCart size={14} /> Buy Station online</div><button onClick={() => setStationOpen(open => !open)} className="border border-cyan-300/40 bg-cyan-400/10 px-2 py-1 text-[9px] font-black uppercase text-cyan-100 hover:bg-cyan-400/20">{stationOpen ? 'Close' : 'Shop'}</button></div>
        {!stationOpen && <p className="mt-2 text-[10px] text-white/50">Regroup, restock, and build your support loadout. Credits are personal.</p>}
      ...
      {!isSpectator && hud.lifeState === 'downed' && <div className="pointer-events-none absolute inset-0 z-30 flex items-center justify-center bg-black/35"><div className="w-[min(420px,calc(100vw-2rem))] border border-amber-300/50 bg-black/80 p-6 text-center backdrop-blur-md"><div className="text-xs font-black uppercase tracking-[0.3em] text-amber-200">You are downed</div><div className="mt-3 text-4xl font-black text-white">{Math.ceil(hud.downedRemainingMs / 1000)}s</div><div className="mt-3 text-xs text-white/60">A teammate must stand close and hold <span className="font-black text-cyan-200">F</span> for 3 seconds.</div>{hud.reviverId && <div className="mt-3 text-[10px] font-bold uppercase tracking-wider text-emerald-200">Revive in progress · {Math.round(hud.reviveProgressMs / 3000 * 100)}%</div>}<div className="mt-2 h-1.5 overflow-hidden bg-white/10"><div className="h-full bg-cyan-300 transition-[width]" style={{ width: `${Math.max(0, hud.reviveProgressMs / 3000 * 100)}%` }} /></div></div></div>}
